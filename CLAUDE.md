...
```bash
uv sync                    # 依存インストール（CPU PyTorch / Python 3.11-3.13）
...
src/
  ...
  recorder.py      # 録音 + VAD（sounddevice + silero-vad）
...
- 通常の `uv sync` は CPU PyTorch を使う。CUDA 依存は `cuda` / `qwen3-cuda` extra で明示する。
...
`2d4d4762e7ee294ac8e47a177ac2e9b0e8d0d43f` に固定し、Windows/Python 3.13 の
ORT API不整合を避けるため `sherpa-onnx==1.13.1` と `sherpa-onnx-core==1.13.1` も固定する。
